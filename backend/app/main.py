    ...
    try:
        ...
        async with AsyncSessionLocal() as db:
            ...
            try:
                ...
                stamp = (await db.execute(_sel(GameSetting).where(GameSetting.key == "freq_resync_v3"))).scalar_one_or_none()
                if stamp is None:
                    updated = await resync_flags_from_json(db)
                    db.add(GameSetting(key="freq_resync_v3", value="done"))
                    await db.commit()
                    print(f"[startup] Frekans resync v3: {updated} kelime bayrağı güncellendi.")
            except Exception as e:
